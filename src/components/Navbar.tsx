...
import { User, ShoppingCart, X, Minus, Plus, ArrowRight } from 'lucide-react';
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          >
            <div className="w-5 h-5 relative flex items-center justify-center">
              <span className={`absolute w-4 h-0.5 bg-gray-700 rounded-full transition-all duration-300 ${isMobileMenuOpen ? 'rotate-45' : '-translate-y-1.5'}`} />
              <span className={`absolute w-4 h-0.5 bg-gray-700 rounded-full transition-all duration-300 ${isMobileMenuOpen ? 'opacity-0 scale-0' : ''}`} />
              <span className={`absolute w-4 h-0.5 bg-gray-700 rounded-full transition-all duration-300 ${isMobileMenuOpen ? '-rotate-45' : 'translate-y-1.5'}`} />
            </div>
          </button>
      ...
      >
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto">
              ...
              <nav className="space-y-1">
                <Link
                  ...
                  className={`flex items-center justify-between py-3 px-4 rounded-lg transition-all ${
                    isHomePage ? 'bg-brand-green-dark text-white' : 'text-gray-700 hover:bg-gray-50'
                  }`}
                >
                  <span className="font-medium">Home</span>
                  <ArrowRight className={`w-4 h-4 ${isHomePage ? 'text-white' : 'text-gray-400'}`} />
                </Link>
                <Link
                  to="/shop"
                  onClick={() => setIsMobileMenuOpen(false)}
                  className={`flex items-center justify-between py-3 px-4 rounded-lg transition-all ${
                    isShopPage ? 'bg-brand-green-dark text-white' : 'text-gray-700 hover:bg-gray-50'
                  }`}
                >
                  <span className="font-medium">Shop</span>
                  <ArrowRight className={`w-4 h-4 ${isShopPage ? 'text-white' : 'text-gray-400'}`} />
                </Link>
                <Link
                  to="/services"
                  onClick={() => setIsMobileMenuOpen(false)}
                  className={`flex items-center justify-between py-3 px-4 rounded-lg transition-all ${
                    isServicesPage ? 'bg-brand-green-dark text-white' : 'text-gray-700 hover:bg-gray-50'
                  }`}
                >
                  <span className="font-medium">Services</span>
                  <ArrowRight className={`w-4 h-4 ${isServicesPage ? 'text-white' : 'text-gray-400'}`} />
                </Link>
                <Link
                  to="/about"
                  onClick={() => setIsMobileMenuOpen(false)}
                  className={`flex items-center justify-between py-3 px-4 rounded-lg transition-all ${
                    isAboutPage ? 'bg-brand-green-dark text-white' : 'text-gray-700 hover:bg-gray-50'
                  }`}
                >
                  <span className="font-medium">About</span>
                  <ArrowRight className={`w-4 h-4 ${isAboutPage ? 'text-white' : 'text-gray-400'}`} />
                </Link>
                <Link
                  to="/contact"
                  onClick={() => setIsMobileMenuOpen(false)}
                  className={`flex items-center justify-between py-3 px-4 rounded-lg transition-all ${
                    isContactPage ? 'bg-brand-green-dark text-white' : 'text-gray-700 hover:bg-gray-50'
                  }`}
                >
                  <span className="font-medium">Contact</span>
                  <ArrowRight className={`w-4 h-4 ${isContactPage ? 'text-white' : 'text-gray-400'}`} />
                </Link>
              </nav>
            </div>
            ...
            {/* Vendor Portal */}
            <div className="px-4 py-5">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3 px-4">Account</p>
              <a
                href="https://bloom.greenstonerx.com/register"
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-3 py-3 px-4 rounded-lg text-gray-700 hover:bg-gray-50 transition-all"
              >
                <div className="w-9 h-9 rounded-full bg-gray-100 flex items-center justify-center flex-shrink-0">
                  <User className="w-4 h-4 text-gray-600" />
                </div>
                <div className="min-w-0">
                  <p className="font-medium text-sm">Vendor Portal</p>
                  <p className="text-xs text-gray-500">Login or register</p>
                </div>
              </a>
            </div>
          </div>
          ...
          <div className="p-4 border-t border-gray-100 bg-white">
            <a
              href="tel:+18556257588"
              className="block w-full bg-gradient-to-r from-brand-green-light via-brand-green-primary to-brand-green-dark text-white py-3 rounded-full font-medium text-center text-sm hover:opacity-90 transition-opacity"
            >
              Call (855) 625-7588
            </a>
          </div>
